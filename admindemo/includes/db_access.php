...
class Databases 
{
    ...
    public function __construct() {
        ...
        if ($this->conn->connect_errno) {            
            writelog("[MYSQL] Failed to dbconnect to MySQL: (" . $this->conn->dbconnect_errno . ") " . $this->conn->connect_error);
            //writelog(date('m/d/Y h:i:s a', time()), "Failed to dbconnect to MySQL:(" . $this->dbconn->connect_errno . ")" . $this->dbconn->connect_error);
        }else{
            writelog("[MYSQL] Connected to database");
        }
    }
    ...
    function runQuery($query)
    {
        ...
        $this->res = mysqli_query($this->conn,$query,MYSQLI_STORE_RESULT);
         ...
         {                
                writelog("[MYSQL] Failed to run query. MySQL error: " .mysqli_error($this->conn));
         }
        return $this->res;
    } 
